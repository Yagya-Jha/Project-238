...
#setup blueprint for api
api = Blueprint('api', __name__, url_prefix="/api")

#get path for folder to upload to
UPLOAD_FOLDER = os.path.abspath("app/static/attachments")
#setting login route to POST
@api.route('/login', methods=['POST'])
#Login Function-> matching userinfo via sql query
def login():
    try:
...
#setting add-address' route to POST
...
#setting create-order's method to POST
@api.route("/create-order", methods=["POST"])
#(Function) create Order by adding to database
def create_order():
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
...
#setting submit-help's method to POST
@api.route("/submit-help", methods=["POST"])
#(Function) submit help via post method and adding info to database
def submit_help():
...
#Function for downloading the uploaded file (in help section)
@api.route("/download/<path:filename>")
def download(filename):
    return send_file(os.path.join(UPLOAD_FOLDER, filename), as_attachment=True)

#Setting route to search-order (to be able to search among orders [From Database])
...
#Code to execute the searches in abouve function(s)
...
#Function to get the customer through SQL injection
