...
#recreating the database i.e->deleting content in it and emptying it.
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()
#Reading code from csv files and then getting required info. from product,user,customer,supplier,companyProducts, companyOrders, orderItems
def seeder():
...
@cli.command()
#calling recreate database and seeder function
def rsd():
